...
class PokedexHolder extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.pokemons.list !== nextProps.pokemons.list) {
      this.setState({
        items: nextProps.pokemons.list.slice(
          this.state.params.offset,
          this.state.params.limit + this.state.params.offset
        )
      });
    }
  ...
  //       params.offset = Math.abs(params.limit + params.offset);
  ...
  fetchData = i => {
    const state = { ...this.state };
    const { params, paginate } = state;
    params.offset = i === 0 ? 0 : i * paginate;
    params.limit = paginate;
    const data = this.props.pokemons.list.slice(
      params.offset,
